	...
	<body>
		...
		<div id="show-test" class="content scaffold-show" role="main">
			...
			<ol class="property-list test">
				...
				<g:if test="${testInstance?.books}">
				<li class="fieldcontain">
					<span id="books-label" class="property-label"><g:message code="test.books.label" default="Books" /></span>
					
						<g:each in="${testInstance.books}" var="b">
						<span class="property-value" aria-labelledby="books-label"><g:link controller="subTest" action="show" id="${b.id}">${b?.encodeAsHTML()}</g:link></span>
						</g:each>
